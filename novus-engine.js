...
rel:{safety:.50,trust:.50,bond:.50,connection:.50,meaning:.50,honesty:.50,stability:.50,intimacy:.40,growth:.50,tension:.08,friction:.05,overall:.50,passion:.50,commitment:.50,sternIntimacy:.50},
...
E.calcRelHealth=function(){var r=S.rel;var posW={safety:.14,trust:.13,bond:.11,connection:.09,meaning:.07,honesty:.08,stability:.08,intimacy:.06,growth:.06,passion:.05,commitment:.06,sternIntimacy:.07};var sum=0;var keys=Object.keys(posW);for(var i=0;i<keys.length;i++){sum+=posW[keys[i]]*(r[keys[i]]||0);}sum-=(r.tension||0)*.12;sum-=(r.friction||0)*.08;r.overall=cl(sum);var rb=E.calcRusbult();r.commitment=rb.commit;return r.overall;};
...
E.applyPractice=function(who,actId){E.pushUndo();var p=null;for(var pi=0;pi<D.PERSONAL_POS.length;pi++){if(D.PERSONAL_POS[pi].id===actId){p=D.PERSONAL_POS[pi];break;}}if(!p)return;savePrev(who);saveRelPrev();var other=who==="a"?"b":"a";if(p.effects){var ek=Object.keys(p.effects);for(var i=0;i<ek.length;i++){if(S[who][ek[i]]!==undefined)S[who][ek[i]]=cl(S[who][ek[i]]+p.effects[ek[i]]);}}if(p.relSpillover){var rk=Object.keys(p.relSpillover);for(var i=0;i<rk.length;i++){if(S.rel[rk[i]]!==undefined)S.rel[rk[i]]=cl(S.rel[rk[i]]+p.relSpillover[rk[i]]);}}E.calcHealth(who);E.calcHealth(other);E.calcWOT(who);E.calcWOT(other);E.calcSatir(who);E.calcSatir(other);E.calcVagal(who);E.calcVagal(other);E.calcRelHealth();S.sentHistory.push(1);S.evtCount++;E.addLog(S[who].name+" practiced: "+p.label,"ind",who);E.addLog(S[who].name+" \u2192 "+p.label+" (resilience)","rel");playTone("pos");};
E.applyToxic=function(who,actId){E.pushUndo();var t=null;for(var ti=0;ti<D.PERSONAL_NEG.length;ti++){if(D.PERSONAL_NEG[ti].id===actId){t=D.PERSONAL_NEG[ti];break;}}if(!t)return;savePrev(who);saveRelPrev();var other=who==="a"?"b":"a";if(t.effects){var ek=Object.keys(t.effects);for(var i=0;i<ek.length;i++){if(S[who][ek[i]]!==undefined)S[who][ek[i]]=cl(S[who][ek[i]]+t.effects[ek[i]]);}}if(t.partnerEffects){var pk=Object.keys(t.partnerEffects);for(var i=0;i<pk.length;i++){if(S[other][pk[i]]!==undefined)S[other][pk[i]]=cl(S[other][pk[i]]+t.partnerEffects[pk[i]]);}}if(t.relEffects){var rk=Object.keys(t.relEffects);for(var i=0;i<rk.length;i++){if(S.rel[rk[i]]!==undefined)S.rel[rk[i]]=cl(S.rel[rk[i]]+t.relEffects[rk[i]]);}}E.calcHealth(who);E.calcHealth(other);E.calcWOT(who);E.calcWOT(other);E.calcSatir(who);E.calcSatir(other);E.calcVagal(who);E.calcVagal(other);E.calcRelHealth();S.sentHistory.push(-1);S.evtCount++;updateConflict(1);openRepair(t.label);E.addLog(S[who].name+" did: "+t.label,"ind",who);E.addLog(S[who].name+" \u2192 "+t.label+" (toxic)","rel");playTone("neg");};
...
E.addLog((isPositive?"+":"-")+" "+S[who].name+": "+act.label,"rel");E.addLog(act.label+(isPositive?" (growth)":" (risk)"),"ind",who);
return entry;};
E.applyMoodEffects=function(who){var p=S[who];var mood=D.MOOD[p.mood];if(!mood)return;var base={emotionRegulation:0,selfWorth:0,hope:0,externalStress:0,rejectionSensitivity:0,secureWithPartner:0};if(mood.r)base.rejectionSensitivity=mood.r*.15;if(mood.a)base.secureWithPartner=mood.a*.12;if(mood.sr)base.emotionRegulation=mood.sr*.12;if(mood.g)base.hope=mood.g*.10;var circ=D.CIRCUMPLEX[p.mood];if(circ){if(circ.v<-.3)base.selfWorth+=circ.v*.05;if(circ.ar>.5)base.externalStress+=circ.ar*.04;}var bk=Object.keys(base);for(var i=0;i<bk.length;i++){if(base[bk[i]]&&S[who][bk[i]]!==undefined)S[who][bk[i]]=cl(S[who][bk[i]]+base[bk[i]]);}};
E.applyFinComfort=function(who){var p=S[who];var fc=(p.finComfort||50)/100;var stressMod=(fc-.5)*-.08;p.externalStress=cl(p.externalStress+stressMod);p.selfWorth=cl(p.selfWorth+(fc-.5)*.04);};
E.calcRelOverall=E.calcRelHealth;
E.randomize=function(who){var p=S[who];var mbtis=Object.keys(D.MBTI);p.mbti=mbtis[Math.floor(Math.random()*mbtis.length)];var enns=Object.keys(D.ENNEA);p.ennea=enns[Math.floor(Math.random()*enns.length)];var atts=Object.keys(D.ATT);p.attach=atts[Math.floor(Math.random()*atts.length)];var lls=D.LL_LIST;p.llGive=lls[Math.floor(Math.random()*lls.length)];p.llReceive=lls[Math.floor(Math.random()*lls.length)];var cons=Object.keys(D.CONFLICT);p.conflict=cons[Math.floor(Math.random()*cons.length)];var moods=Object.keys(D.MOOD);p.mood=moods[Math.floor(Math.random()*moods.length)];p.support=Math.floor(Math.random()*100);p.finComfort=Math.floor(Math.random()*100);E.setOceanFromMBTI(who);p.ocean.O=Math.round((p.ocean.O+Math.random()*.2-.1)*100)/100;p.ocean.C=Math.round((p.ocean.C+Math.random()*.2-.1)*100)/100;p.ocean.E=Math.round((p.ocean.E+Math.random()*.2-.1)*100)/100;p.ocean.A=Math.round((p.ocean.A+Math.random()*.2-.1)*100)/100;p.ocean.N=Math.round((p.ocean.N+Math.random()*.2-.1)*100)/100;E.recalcAll();};
N.E=E;})(window.Novus);
