...
E.calcRelHealth=function(){var r=S.rel;var pw={safety:.15,trust:.14,bond:.11,connection:.09,meaning:.07,honesty:.08,stability:.08,intimacy:.06,growth:.06,passion:.05,commitment:.06,sternIntimacy:.05};var sum=0;var keys=Object.keys(pw);for(var i=0;i<keys.length;i++){sum+=pw[keys[i]]*(r[keys[i]]||0);}sum-=(r.tension||0)*.12+(r.friction||0)*.10;r.overall=cl(sum);var rb=E.calcRusbult();r.commitment=rb.commit;return r.overall;};
...
E.applyMoodEffects=function(who){var p=S[who];var m=D.MOOD[p.mood];if(!m)return;var keys=["emotionRegulation","selfWorth","secureWithPartner","hope","rejectionSensitivity","trustInPartner","externalStress"];for(var i=0;i<keys.length;i++){var k=keys[i];var delta=0;if(k==="emotionRegulation")delta=((m.sr||0)+(m.res||0))/2*.5;else if(k==="selfWorth")delta=(m.res||0)*.3;else if(k==="secureWithPartner")delta=(m.a||0)*.3;else if(k==="hope")delta=(m.g||0)*.3;else if(k==="rejectionSensitivity")delta=(m.r||0)*.3;else if(k==="externalStress")delta=-(m.res||0)*.2;if(delta)p[k]=cl(p[k]+delta);}E.calcHealth(who);E.calcWOT(who);E.calcSatir(who);E.calcVagal(who);};
E.applyFinComfort=function(who){var p=S[who];var fc=(p.finComfort||60)/100;var stress=(1-fc)*.15;p.externalStress=cl(p.externalStress+stress-.075);E.calcHealth(who);};
E.processPersonalActivity=function(who,type,idx){if(type==="pos")E.applyPractice(who,idx);else E.applyToxic(who,idx);};

E.randomize=function(who){var mbtiKeys=Object.keys(D.MBTI);S[who].mbti=mbtiKeys[Math.floor(Math.random()*mbtiKeys.length)];var enneaKeys=Object.keys(D.ENNEA);S[who].ennea=enneaKeys[Math.floor(Math.random()*enneaKeys.length)];var attKeys=Object.keys(D.ATT);S[who].attach=attKeys[Math.floor(Math.random()*attKeys.length)];var llKeys=D.LL_LIST||["time","words","gifts","acts","touch"];S[who].llGive=llKeys[Math.floor(Math.random()*llKeys.length)];S[who].llReceive=llKeys[Math.floor(Math.random()*llKeys.length)];var confKeys=Object.keys(D.CONFLICT);S[who].conflict=confKeys[Math.floor(Math.random()*confKeys.length)];var moodKeys=Object.keys(D.MOOD);S[who].mood=moodKeys[Math.floor(Math.random()*moodKeys.length)];S[who].support=Math.floor(Math.random()*80)+10;S[who].finComfort=Math.floor(Math.random()*80)+10;var dims=["O","C","E","A","N"];for(var i=0;i<dims.length;i++){S[who].ocean[dims[i]]=Math.round((Math.random()*.6+.2)*100)/100;}E.setOceanFromMBTI(who);};
N.E=E;})(window.Novus);
